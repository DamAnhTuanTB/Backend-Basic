...
router.get("/", async (req, res) => {
  const evaluates = await EvaluateModel.find().sort({ createdAt: -1 });

  const result = evaluates.map((evaluate) => {
    return {
      id: evaluate._id,
      productId: evaluate.productId,
      userId: evaluate.userId,
      numberStar: evaluate.numberStar,
      comment: evaluate.comment,
      time: evaluate.time,
      allowVisible: evaluate.allowVisible,
    };
  });
...
//     const data = await EvaluateModel.find({ product: idProduct })
//       .sort(sortCondition)
//       .populate("user")
//       .skip((myPage - 1) * myLimit)
//       .limit(myLimit);
...
router.delete("/:id", (req, res) => {
  ...
  EvaluateModel.deleteOne({ _id: id })
    ...
    .catch((err) =>
      res.status(500).send({ message: "Xóa đánh giá thất bại." })
    );
});
...
router.put("/allow-visible/:id", async (req, res) => {
  try {
    const id = req.params.id;
    const allowVisible = req.body.allowVisible;
